...
model = load_model("heart_disease_model.h5", compile=False)
...
def fetch_data_from_mysql(user_id, data_id):
    # Fetch data from MySQL database based on 'userId' and 'dataId'
...
def save_prediction_to_db(user_id, data_id, prediction_result):
    # Update the 'result' column in the database based on 'userId' and 'dataId'
